...
export const donationAction = (body) => async (dispatch) => {
 ...
 `${process.env.REACT_APP_STRIPE_PUBLISHABLE_KEY}`
  ...
  try {
    const response = await axios.post('https://yumyard-server.cyclic.app/api/donation/donate',body);
...
export const getAllDonations = ()=>async(dispatch)=>{
  ...
  try{
  const response = await axios.get('https://yumyard-server.cyclic.app/api/donation/getalldonations');
...
export const saveThroughSessionDonation = (transactionId)=>async(dispatch)=>{
  ...
  try {
    const response = await axios.post('https://yumyard-server.cyclic.app/api/session-donations/findByIdAndSaveDonation',{transactionId})
...
export const downloadDonationReceiptAction = (transactionId) => async (dispatch) => {
  ...
  try {
    const response = await axios.post('https://yumyard-server.cyclic.app/api/session-donations/downloaddonationreceipt', { transactionId });
