...
export const donationAction = (body) => async (dispatch) => {
 ...
 `${process.env.REACT_APP_STRIPE_PUBLISHABLE_KEY}`
  ...
  try {
    const response = await axios.post('/api/donation/donate',body);
...
export const saveThroughSessionDonation = (transactionId)=>async(dispatch)=>{
  ...
  try {
    const response = await axios.post('/api/session-donations/findByIdAndSaveDonation',{transactionId})
...
export const downloadDonationReceiptAction = (transactionId) => async (dispatch) => {
  ...
  try {
    const response = await axios.post('/api/session-donations/downloaddonationreceipt', { transactionId });
